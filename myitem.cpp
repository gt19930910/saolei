...
{
    boomNum = 0;
    if((j-1 >= 0) && (k-1 >= 0) && (a[j-1][k-1] == 1))//四周八块方块一一检测总共有多少个雷
        boomNum++;
    if((j-1 >= 0) && a[j-1][k] == 1)
        boomNum++;
...
{
    if(event->button() == Qt::LeftButton && flag == 0 && IsflagOrwhiteblock == 0)//鼠标点击此块并且还未插旗
    {
        m_x = this->pos().x();
        m_y = this->pos().y();//获取此块坐标
        setNum(m_y/48, m_x/48);//取[0,0]->[8,8]
